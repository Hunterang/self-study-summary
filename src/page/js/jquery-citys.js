...
+function($) {
 ...
 $.fn.citys = function(options) {
		//定义参数
		var _options = $.extend({
				url : './static/jquery-citys.json',					//省市区json数据地址
    ...
    var timer =setTimeout(()=>{
      initx();
		...
		var initx= function(){
			...
			for(var i in cityx){
				if(patternPro.test(i)){	//添加一级列表数据
					proHtml += "<option value='"+(type=="code"?i:cityx[i])+"' data-code='"+ i +"'>"+ cityx[i] +"</option>";
				}
			}
			//渲染省份一级列表
		...
		function pushHtml(arr){
			var cityxArr = arr,
					html = "";
			for(var i in cityxArr){
				html += "<option value='"+(type=="code"?i:cityx[i])+"' data-code='"+ i +"'>"+ cityx[i] +"</option>";
			}
			return html;
		}
	}
